...
from frontend.utils import add_shadow_effect, show_error_message
...
class EditDataScreen(QWidget):
    ...
    def on_region_changed(self, index):
        ...
        provinces = sorted(self.location_data[self.location_data["RegName"] == selected_region]["ProvName"].unique())
    ...
    def get_selected_codes(self):
        ...
        if selected_region != "-- เลือกภาค --":
            filtered_data = self.location_data[self.location_data["RegName"] == selected_region]
            if not filtered_data.empty:
                codes['RegCode'] = filtered_data["RegCode"].iloc[0]
            
            if selected_province != "-- เลือกจังหวัด --":
                filtered_data = filtered_data[filtered_data["ProvName"] == selected_province]
                if not filtered_data.empty:
                    codes['ProvCode'] = filtered_data["ProvCode"].iloc[0]
                
                if selected_district != "-- เลือกอำเภอ/เขต --":
                    filtered_data = filtered_data[filtered_data["DistName"] == selected_district]
                    if not filtered_data.empty:
                        codes['DistCode'] = filtered_data["DistCode"].iloc[0]
                    
                    if selected_subdistrict != "-- เลือกตำบล/แขวง --":
                        filtered_data = filtered_data[filtered_data["SubDistName"] == selected_subdistrict]
                        if not filtered_data.empty:
    ...
    def search_data(self):
        ...
        if codes['RegCode'] is not None:
            ...
            params.append(int(codes['RegCode']))  # แปลงเป็น int
    
        if codes['ProvCode'] is not None:
            sql_conditions.append("ProvCode = ?")
            params.append(int(codes['ProvCode']))  # แปลงเป็น int
    
        if codes['DistCode'] is not None:
            sql_conditions.append("DistCode = ?")
            params.append(int(codes['DistCode']))  # แปลงเป็น int
    
        if codes['SubDistCode'] is not None:
            sql_conditions.append("SubDistCode = ?")
            params.append(int(codes['SubDistCode']))  # แปลงเป็น int
        
        # If no conditions are selected, show a message and return
        if not sql_conditions:
            show_error_message(self, "Search Error", "Please select at least one criteria.")
            return
        
        # Build the full query
        query = "SELECT EA_Code_15, RegName, ProvName, DistName, SubDistName, AreaName, VilName, HouseNumber FROM pop68T.dbo.r_alldata"
        ...
        try:
            ...
            conn = get_connection()
            if not conn:
                show_error_message(self, "Database Error", "Could not connect to database.")
                return
            ...
            show_error_message(self, "Search Error", f"An error occurred during the search: {str(e)}")
    ...
    def display_results(self, results):
        ...
        if not results:
            show_error_message(self, "Search Results", "No records found matching your criteria.")
            return
